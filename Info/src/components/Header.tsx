...
import { Gem, Menu, MessageCircle, Send, User, X, LayoutDashboard, Building2, Info, Calculator, Phone, Globe } from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ t, lang, setLang, setView, currentView }) => {
  ...
  useEffect(() => {
    ...
    const applyPrice = (price: number, rates: { AED: number; RUB: number }) => {
      ...
      setCurrencyRates({ AED: rates.AED.toFixed(2), RUB: rates.RUB.toFixed(2) });
      const baseline = price * 0.92;
      setYearlyGrowth(Number((((price - baseline) / baseline) * 100).toFixed(1)));
    };
    const fetchPrices = async () => {
      try {
        ...
        if (result.success && result.rates) {
          ...
          const newRates = { AED: Number(result.rates.AED.toFixed(2)) || 3.67, RUB: Number(result.rates.RUB.toFixed(2)) || 91.42 };
          applyPrice(livePrice, newRates);
          localStorage.setItem(CACHE_KEY, JSON.stringify({ timestamp: Date.now(), lastPrice: livePrice, rates: newRates }));
        }
    ...
    <>
      ...
      <header className={`fixed top-8 w-full z-[90] transition-all duration-500 px-6 md:px-12 h-16 flex justify-between items-center ${scrolled ? (isDark ? 'bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5' : 'bg-white/80 backdrop-blur-xl border-b border-black/5') : 'bg-transparent'}`}>
        ...
        <div className="flex items-center gap-4">
          <div className={`flex p-1 rounded-lg border transition-colors duration-500 ${bgColor} ${borderColor}`}>
            {(['RU', 'EN'] as const).map((l) => (
              <button key={l} onClick={() => setLang(l)} className={`px-3 py-1 text-[10px] font-bold rounded transition-all ${lang === l ? 'bg-[#d4af37] text-black shadow-sm' : `${textColor} opacity-40 hover:opacity-100`}`}>{l}</button>
            ))}
          </div>
          ...
          <>
            ...
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed inset-y-0 left-0 w-full max-w-xs z-[160] bg-white border-r border-black/5 p-8 flex flex-col">
              ...
              <nav className="flex flex-col gap-2">
                ...
                <MenuButton icon={<Building2 size={20}/>} label={t.menuCompany} active={currentView === 'company'} onClick={() => handleNavClick('company')} />
                <MenuButton icon={<Info size={20}/>} label={t.menuProject} active={currentView === 'project'} onClick={() => handleNavClick('project')} />
                <MenuButton icon={<Calculator size={20}/>} label={t.menuCalculator} onClick={openCalculator} />
                <div className="h-px bg-black/5 my-6" />
                <MenuButton icon={<Phone size={20}/>} label={t.contactBtn} onClick={() => { setIsMenuOpen(false); setIsManagerModalOpen(true); }} />
      ...
      <AnimatePresence>
        {isManagerModalOpen && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsManagerModalOpen(false)} className="absolute inset-0 bg-black/40 backdrop-blur-md" />
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white p-8 md:p-12 rounded-[2rem] w-full max-w-md border border-black/5 flex flex-col items-center shadow-2xl">
              ...
              <div className="flex flex-col gap-4 w-full">
                <ContactOption icon={<Send size={24} />} label="Telegram" sub={t.managerTelegramSub} href="https://t.me/GoldenShareClub" color="bg-[#0088cc]/10 text-[#0088cc]" />
                <ContactOption icon={<MessageCircle size={24} />} label="WhatsApp" sub={t.managerWhatsappSub} href="https://wa.me/971529657370" color="bg-green-500/10 text-green-500" />
              </div>
            </motion.div>
          </div>
        )}
...
const ContactOption = ({ icon, label, sub, href, color }: { icon: React.ReactNode; label: string; sub: string; href: string; color: string }) => (
  <a href={href} target="_blank" rel="noreferrer" className="flex items-center gap-5 p-5 bg-black/5 border border-black/5 rounded-2xl hover:border-[#d4af37]/40 hover:bg-black/[0.08] transition-all group">
    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${color} group-hover:scale-110 transition-transform`}>{icon}</div>
    <div className="flex flex-col">
